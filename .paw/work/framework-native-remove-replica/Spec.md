...
The migration must preserve the exact authority, fencing, cleanup, and publication guarantees already exercised by the existing implementations while removing pilot-only configuration, evaluation reporting, telemetry, and documentation. Pre-migration pilot and explicit remove execution records may be rejected rather than migrated because the pilot was default-off and the project permits breaking changes, but they must never be silently interpreted as the new production contract.
...
- Keep every representative active persisted-record maximum at or below 49,152 bytes.
- Preserve exactly five representative durable boundaries and six accepted persistence writes.
- Remove pilot-only surfaces and document remove-replica as a production migration.
...
Narrative: As an operator or contributor, I need configuration, examples, telemetry, and documentation to describe the production behavior rather than a retired pilot.
...
- **FR-021**: The migration shall remove remove-specific mode selectors, build-time selection, pilot evaluation projections, experiment-only telemetry, pilot-only conditions/status, and stale public resource/deployment fields. (Stories: P2 pilot removal)
- **FR-022**: Validation and documentation shall report operational and persistence evidence: external effects, passive observations, durable boundaries, accepted writes, active-record maxima and budget, terminal-record and payload sizes, and recovery coverage. (Stories: P2 pilot removal)
...
- **SC-006**: Across at least three representative no-fault samples, every per-run active persisted-record maximum is at most 49,152 bytes. (FR-013, FR-015)
...
- The compact 49,152-byte gate is an evidence-backed acceptance budget for the representative production workflow.
