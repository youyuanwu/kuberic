...
status, condition, telemetry, evaluation, and documentation surface.
...
- Project documentation, generated CRD schema, CI, example features,
  conditions, and pilot-era reporting still describe a non-production path.
...
- **`kuberic-operator/src/reconciler.rs`**:
  ...
  - Remove explicit-versus-pilot selection, feature rejection, dual recovery
    routing, pilot conditions, process-summary pilot telemetry, and
...
- **Retired non-operational measurement automation under `scripts/`**:
  - Remove the obsolete automation and its tests without replacement.
- **Associated marker comments and documentation**:
  - Remove marker comments, CI gates, and documentation commands that exist
    only for the retired automation.
...
- [x] Bounds/admission matrix passes:
  `cargo test -p kuberic-operator framework_native_remove_replica`
- [x] The retired automation, CI gate, documentation commands, and marker
  comments are absent.
...
- [x] CI contains no remove pilot feature gate.
...
- **`durable-execution/README.md`**:
  - Replace the retired remove pilot material with the production native
    consumer, independent active/terminal bounds, and retained owner/cleanup
    model.
- **`docs/features/kuberic/durable-execution-roadmap.md`**:
  - Mark remove-replica as graduated, remove pilot-era evaluation language,
    and report final representative operational measurements with
