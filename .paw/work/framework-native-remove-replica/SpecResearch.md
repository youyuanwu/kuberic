...
### Question 6: What do the authoritative merged PR #59 operational and persistence measurements report?
...
**Evidence**: Direct execution of the exact three-sample fixture on
2026-09-07, plus the merged persistence-measurement sources.
...
### Question 7: Which documentation and examples still expose the pilot as a current product path?

**Answer**:

- The durable-execution README says both pilots are default-off and explicit is
  the runtime default (`durable-execution/README.md:403-420`).
- The roadmap describes both explicit implementations as retained defaults and
  records pilot-era remove persistence measurements
...
and example feature manifest.

**Implications**: Production migration documentation no longer needs the
explicit-versus-pilot choice or pilot-era evaluation framing. Stable
checkpoint capacity and terminal bounds remain behavioral constraints.
...
- Standard CI runs `cargo check --all-targets`, formatting, clippy with
  warnings denied, targeted durable workflow gates, `cargo build --all-targets`,
  a Kind job, image building/loading, and finally
