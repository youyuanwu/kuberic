# WorkflowContext

Work Title: Durable Remove Replica
Work ID: durable-remove-replica
Base Branch: main
Target Branch: feature/durable-remove-replica
Execution Mode: current-checkout
Repository Identity: github.com/youyuanwu/kuberic@3d2f129bf328d7869c998718fa98e4dac441693b
Execution Binding: none
Workflow Mode: full
Review Strategy: local
Review Policy: milestones
Session Policy: continuous
Final Agent Review: enabled
Final Review Mode: single-model
Final Review Interactive: smart
Final Review Models: gpt-5.6-sol
Final Review Specialists: all
Final Review Interaction Mode: parallel
Final Review Specialist Models: none
Final Review Perspectives: none
Final Review Perspective Cap: 2
Implementation Model: gpt-5.6-sol
Plan Generation Mode: single-model
Plan Generation Models: gpt-5.6-sol
Planning Docs Review: enabled
Planning Review Mode: single-model
Planning Review Interactive: smart
Planning Review Models: gpt-5.6-sol
Planning Review Specialists: all
Planning Review Interaction Mode: parallel
Planning Review Specialist Models: none
Planning Review Perspectives: none
Planning Review Perspective Cap: 2
Custom Workflow Instructions: none
Initial Prompt: Port remove_replica onto the durable-execution kernel as the second kernel-hosted workflow; measure amortization honestly; correct stale feasibility and roadmap documentation; preserve existing explicit behavior by default.
Issue URL: none
Remote: origin
Artifact Lifecycle: commit-and-clean
Artifact Paths: auto-derived
Additional Inputs: Single-model execution only; all plan generation and reviews use gpt-5.6-sol; no multi-model or Society-of-Thought execution.
