...
#[cfg(feature = "durable-remove-replica-pilot")]
use kuberic_operator::crd::DurableOperationStatus;
#[cfg(feature = "durable-remove-replica-pilot")]
...
#[cfg(feature = "durable-remove-replica-pilot")]
async fn framework_native_remove_status(
    api: &KvClusterApi,
    name: &str,
    status: KubericSetStatus,
    mode: DurableRemoveMode,
) -> KubericSetStatus {
    let snapshot = status.stable_snapshot.clone().unwrap();
    let target = snapshot
        .members
        .iter()
    ...
    let member = status
        .members
        .iter()
        .find(|member| member.id == target.id)
        .unwrap()
        .clone();
    let agent_generation = live_replica_statuses(api, name, 3)
        .await
        .into_iter()
...
#[cfg(feature = "durable-remove-replica-pilot")]
async fn current_framework_native_remove_operation(
    store: &kuberic_durable_execution::InMemoryCheckpointStore,
    status: &KubericSetStatus,
) -> Option<DurableOperationStatus> {
    use kuberic_durable_execution::CheckpointStore;
    let reference = status.remove_replica_execution.as_ref()?;
    let execution =
        kuberic_operator::durable::remove_replica_execution::execution_spec(reference).ok()?;
    let stored = store.load(execution.execution_id()).await.ok()??;
    let payload = stored
        .checkpoint()
        .decode_and_validate(
            &execution,
            kuberic_operator::durable::remove_replica_execution::checkpoint_limits(),
        )
        .ok()?;
    kuberic_operator::durable::remove_replica_execution::replay_active_operation(
        reference,
        payload.active_activities()?,
    )
    .ok()
}

#[cfg(feature = "durable-remove-replica-pilot")]
async fn framework_native_checkpoint_is_terminal(
    store: &kuberic_durable_execution::InMemoryCheckpointStore,
    status: &KubericSetStatus,
) -> bool {
    use kuberic_durable_execution::CheckpointStore;
    let Some(reference) = status.remove_replica_execution.as_ref() else {
        return false;
    };
    let Ok(execution) =
        kuberic_operator::durable::remove_replica_execution::execution_spec(reference)
    else {
        return false;
    };
    store
        ...
        .is_some_and(|payload| payload.terminal_outcome().is_some())
}

#[cfg(feature = "durable-remove-replica-pilot")]
async fn framework_native_delete_is_exposed(
    store: &kuberic_durable_execution::InMemoryCheckpointStore,
    status: &KubericSetStatus,
) -> bool {
    ...
    let Some(stored) = store.load(execution.execution_id()).await.ok().flatten() else {
        return false;
    };
    let Ok(payload) = stored.checkpoint().decode_and_validate(
        &execution,
        kuberic_operator::durable::remove_replica_execution::checkpoint_limits(),
    ) else {
        return false;
    ...
    matches!(last.state(), ActivityState::DispatchExposed { .. })
        && kuberic_operator::durable::remove_replica_execution::decode_boundary_input(last.input())
            .is_ok_and(|input| {
                matches!(
                    input,
                    kuberic_operator::durable::remove_replica_execution::RemoveReplicaBoundaryInput::DeleteCommand { .. }
                )
            })
}
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_happy_path() {
    let api = KvClusterApi::new();
    let bootstrap = ReconcilerState::default();
    let healthy = create_healthy_set(&api, &bootstrap, "framework-native-remove-happy", 3).await;
    let accepted = framework_native_remove_status(
        &api,
        "framework-native-remove-happy",
        healthy,
    ...
    api.reset_operations();
    let completed =
        drive_framework_native_remove(&api, &state, "framework-native-remove-happy", accepted)
            .await;
    assert_eq!(completed.phase, Phase::Healthy);
    assert_eq!(completed.stable_snapshot.as_ref().unwrap().members.len(), 2);
    assert!(completed.operation.is_none());
    assert!(completed.durable_remove_replica_pilot.is_none());
    assert_eq!(
        completed
    ...
    use kuberic_durable_execution::CheckpointStore;
    let execution =
        kuberic_operator::durable::remove_replica_execution::execution_spec(&reference).unwrap();
    let stored = store.load(execution.execution_id()).await.unwrap().unwrap();
    let payload = stored
        .checkpoint()
        .decode_and_validate(
        ...
        .unwrap();
    assert!(payload.terminal_outcome().is_some());
}

#[cfg(feature = "durable-remove-replica-pilot")]
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_every_boundary_restart() {
    let api = KvClusterApi::new();
    let healthy = create_healthy_set(
        &api,
        &ReconcilerState::default(),
        "native-remove-restart",
        3,
    )
    .await;
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_lost_reply_and_quarantine() {
    let api = KvClusterApi::new();
    let healthy = create_healthy_set(
        &api,
        &ReconcilerState::default(),
        "native-remove-lost-reply",
        3,
    )
    .await;
    let accepted = framework_native_remove_status(
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_conflict_and_unknown_write_reload() {
    for (name, fault) in [
        (
            "native-remove-conflict",
            kuberic_durable_execution::InMemoryFault::ConflictWithoutApply,
        ),
        (
            "native-remove-unknown",
            kuberic_durable_execution::InMemoryFault::OutcomeUnknownAfterApply,
        ),
    ] {
        let api = KvClusterApi::new();
        let healthy = create_healthy_set(&api, &ReconcilerState::default(), name, 3).await;
        let accepted =
        ...
        reconcile_set(&make_set(name, 2, Some(accepted)), &api, &state)
            .await
            .unwrap();
        let reloading = api.last_status().unwrap();
        assert!(reloading.conditions.iter().any(|condition| {
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_uid_fences_replacement() {
    let api = KvClusterApi::new();
    let healthy = create_healthy_set(
        &api,
        &ReconcilerState::default(),
        "native-remove-uid-fence",
        3,
    )
    .await;
    let accepted = framework_native_remove_status(
    ...
    .await;
    let target_name = "native-remove-uid-fence-2";
    api.crash_pod(target_name);
    ...
    api.reset_operations();
    reconcile_set(
        &make_set("native-remove-uid-fence", 2, Some(accepted)),
        &api,
        &state,
    )
    .await
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_exact_primary_status_gap_has_no_churn() {
    let api = KvClusterApi::new();
    let healthy = create_healthy_set(
        &api,
        &ReconcilerState::default(),
        "native-remove-primary-gap",
        3,
    )
    .await;
    let accepted = framework_native_remove_status(
    ...
    api.fail_next_status(InjectedStatusError::Unavailable);
    reconcile_set(
        &make_set("native-remove-primary-gap", 2, Some(accepted)),
        &api,
        &state,
    )
    .await
    ...
    api.fail_next_status(InjectedStatusError::Unavailable);
    reconcile_set(
        &make_set("native-remove-primary-gap", 2, Some(waiting)),
        &api,
        &state,
    )
    .await
    ...
    api.fail_next_status(InjectedStatusError::Unavailable);
    reconcile_set(
        &make_set("native-remove-primary-gap", 2, Some(waiting.clone())),
        &api,
        &state,
    )
    .await
    ...
    assert_eq!(
        ...
        status_count,
        "primary gap status changed to {:?}",
        api.last_status().unwrap().conditions
    );
    assert_eq!(
        waiting
            .remove_replica_execution
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_exact_target_status_gap_has_no_churn() {
    let api = KvClusterApi::new();
    let healthy = create_healthy_set(
        &api,
        &ReconcilerState::default(),
        "native-remove-target-gap",
        3,
    )
    .await;
    let target = "native-remove-target-gap-2";
    let accepted = framework_native_remove_status(
        &api,
        "native-remove-target-gap",
        healthy,
    ...
    api.crash_pod(target);
    reconcile_set(
        &make_set("native-remove-target-gap", 2, Some(accepted)),
        &api,
        &state,
    )
    .await
    ...
    let waiting = api.last_status().unwrap();
    reconcile_set(
        &make_set("native-remove-target-gap", 2, Some(waiting)),
        &api,
        &state,
    )
    .await
    ...
    let execution_id = waiting
        ...
        .clone();
    reconcile_set(
        &make_set("native-remove-target-gap", 2, Some(waiting.clone())),
        &api,
        &state,
    )
    .await
...
#[test_log::test(tokio::test)]
#[serial]
async fn test_framework_native_remove_replica_terminal_precedes_status_publication() {
    let api = KvClusterApi::new();
    let healthy = create_healthy_set(
        &api,
        &ReconcilerState::default(),
        "native-remove-terminal-order",
        3,
    )
    .await;
    let mut status = framework_native_remove_status(
    ...
    for _ in 0..240 {
            ...
            })
        {
            ...
            .await
            .unwrap();
            assert!(framework_native_delete_is_exposed(&store, &status).await);
            reconcile_set(
                &make_set("native-remove-terminal-order", 2, Some(status.clone())),
                &api,
                &state,
